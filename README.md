...
깃헙 페이지에 ~~아무렇게나~~ 쓴 **마크다운 문서**를 비주얼 노벨로 변환해 줍니다. 비주얼 노벨로 읽지 않고 마크다운 페이지에서 읽어도 불편함이 없도록 하는 것을 목표로 하고 있습니다.
...
태그에는 총 두 종류가 있습니다. 첫번째는 마크다운 문서에 접근하는 유저들에게도 보이는 부분으로, 태그가 변경되었음을 알리는 `` 태그, 두번째는 유저들에게는 보이지 않는 부분으로, 이전 페이지의 태그를 다음 페이지에도 유지하겠다는 의미의 `<!-- -->` 태그입니다. (효과음은 유지할 수 없습니다.)
...
만약 캐릭터의 표정이나 효과 등이 바뀐다면 `` 태그를 다시 사용하여 지정해 주셔야 합니다.
